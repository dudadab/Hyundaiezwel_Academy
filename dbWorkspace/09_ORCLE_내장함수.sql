...
SELECT CLIENTNO, ROUND(AVG(BOOKPRICE*BSQTY)) AS 평균주문액,
                 ...
                 ROUND(AVG(BOOKPRICE*BSQTY),-1) AS "10의 자리까지 평균주문액",
                 ROUND(AVG(BOOKPRICE*BSQTY),-2) AS "100의 자리까지 평균주문액",
                 ROUND(AVG(BOOKPRICE*BSQTY),-3) AS "1000 자리까지평균주문액"
FROM BOOK, BOOKSALE
...
-- CONCAT / || : 문자열 연결
-- 데이터베이스에 저장은 리터럴로 저장됨 -> 저자 : 도서명 : 가격 형식의 데이터가 개발 시 필요

...
-- 서점의 모든 도서는 회원가입되어 있는 회원이 서점에 등록돠어 있는 도서를 구매할 수 있다 : 구매관계테이블(BOOKSALE)
...
-- '%길%' : 길을 포함하는 문자열
-- '%동' : 동으로 끝나는 문자열
-- ____ : 4개의 문자로 구성된 문자열 (한 개의 문자를 의미하는 _가 4개 이기 때문)
...
-- 기준열을 두개 이상 나열 가능 - 우선기준, 두번째 기준, 세번째 기준
-- ASC : 오름차순(생략가능)
-- DESC : 내림차순
-- 도서정보를 도서명 순으로 반환
SELECT * FROM BOOK
...
-- 정렬 조건 2개 이상일 경우
-- 도서재고를 기준으로 내림차순 정렬하고 재고가 동일한 튜플인 경우 저자를 기준으로 오름차순한 도서정보 반환
SELECT * FROM BOOK
ORDER BY BOOKSTICK DESC, BOOKAUTHOR ASC;
...
-- 모든 컬럼은 컬럼의 별명 생성 가능(SELECT문에서 AS 활용해서)
-- 컬럼 AS "별명"
...
-- 성별에 따른 고객 수
select clientAddress as 성별, count(*) as 고객수, clientAddress as 지역명
from client

...
// View 별로 컨트롤러 각각 구성해서 사용하면 DAO 접근 통로가 많아지고
// db 사용 제어가 불가능함
// 싱글톤으로 구성 : 단 하나만 객체 생성 - 공유해서 사용
public class Controller {
	// 본인 객체 인스턴스를 본인이 제공 - private 객체 구성
	...
	public static Controller getInstance() {
		return instance; // 호출되면 공유 instance 참조값을 반환
	}
	////////// 데이터 처리 method //////////
	BookDAO dao = new BookDAO(); // BookDAO가 갖고 있는 모든 메서드 사용 가능
	IBookDAO iDao = new BookDAO(); // IBookDAO의 메서드만 사용 가능
	// DAO의 메서드들은 예외를 호출하는 쪽으로 throw 진행함 - controller가 예외처리 해줘야 함
	...
	public void delete(String bookNo) {
		try {
			...
			if(dao.delete(dto)) {
				ResultView.succMsg(bookNo + "도서 정보가 삭제되었습니다.");
			}
		}catch(Exception e) {
			e.printStackTrace();
			ResultView.errorMsg("도서 정보가 삭제 오류");
		}
	}
}

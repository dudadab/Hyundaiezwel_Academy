...
public class InsertBookMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// preparedStatement 객체 생성 시 바인딩 될 쿼리구문 전달
			pstmt = con.prepareStatement(sql);
			// 바인딩 될 data 준비 되면, 실제값을 바인딩
			// PerpareStatement.setXXX(idx, 값) -> 해당 위치 idx(?의 순서값)에 값을 바인딩
			// XXX는 바인딩될 실제값의 타입
			pstmt.setString(1, bookNo);
			pstmt.setString(2, bookName);
			pstmt.setString(bookStock, sql);
			...
			// insert문은 반환되는 결과가 수행된 ㅎ ㅚㅅ수 : 저장된 행 수 반환
			// 필요없으면 저장하지 않아도 됨

 ...
 */
 ...
 window.onload = function(){
	...
	if(navigator.mediaDevices){
		...
		let chunks = []; //녹음데이터 저장을 위한 변수
		//mediaDevices 카메라 마이크 공유화면 등 현재 연결된 미디어 입력장치로의 접근 방법 제공 인터페이스
		navigator.mediaDevices.getUserMedia(constraints)
			.then((stream)=>{
				//녹음을 위한 객체 생성 MediaRecorder 클래스 사용해서 녹음 객체 생성 stream 객체 생성자에 전달해야 함
				...
				mediaRecoder.onstop = e => {
					...
					//sound-clips 태그 내에 이전에 포함된 자식노드 있으면 제거
					if(soundClips.hasCildNodes()){
					...
					const clipName = "voiceRecord"; //다운로드 시 사용할 파일이름
					...
					a.download = clipName;
					a.click(); //a태그에 클릭이벤트 발생 자동 다운로드 진행
					
				} //mediaReccorder.onstop 끝
